...
public class Homework_4 {
    public static class Player {
        ...
        public void start(Cell cell) {
            if (cell.start)
                System.out.print("Старт в точке " + "[" + cell.x + ", " + cell.y + "]");
    ...
    public static class Cell {
        ...
        public void setNeighbours(Maze maze) {
            if (maze.cell[this.x + 1][this.y].space > 0)
                this.neighbours[0] = maze.cell[this.x + 1][this.y];
            if (maze.cell[this.x - 1][this.y].space > 0)
                this.neighbours[1] = maze.cell[this.x - 1][this.y];
            if (maze.cell[this.x][this.y + 1].space > 0)
                this.neighbours[2] = maze.cell[this.x][this.y + 1];
            if (maze.cell[this.x][this.y - 1].space > 0)
                this.neighbours[3] = maze.cell[this.x][this.y - 1];
        }
    }
    ...
    public static class Maze {
        ...
        public Maze(int size) {
            ...
            for (int i = 0; i < this.size; i++)
                for (int j = 0; j < this.size; j++) {
                    ...
                    // System.out.println("x - " + j + " y - " + i + " " + this.cell[j][i].x + " " +
                    // this.cell[j][i].y);
            ...
            for (int i = 1; i < size - 2; i++)
                for (int j = 1; j < size - 2; j++) {
                    if (this.cell[i][j - 1].space == 1 && this.cell[i][j + 1].space == 1)
                        this.cell[i][j].space = 1;
                }
            path = new ArrayList<>();
        }
        ...
        public void wallBreaker() {
            int x, y;
            int[][] steps = { { -1, 0, 1, 2 }, { 1, 0, 2, 1 }, { -1, 0, 2, 1 }, { 1, 0, 1, 2 }, { 0, -1, 1, 2 },
                    { 0, 1, 2, 1 }, { 0, -1, 2, 1 }, { 0, 1, 1, 2 } };
            for (y = 1; y < this.size - 1; y++)
                for (x = 1; x < this.size - 1; x++)
                    if (this.cell[x][y].space == 0)
                        for (int[] step : steps)
                            if (this.cell[x + step[0]][y + step[1]].space == step[2]
                                    && this.cell[x - step[0]][y - step[1]].space == step[3]) {
        ...
        public int randomQuarter() {
            ...
            if (quarter)
                variable = rnd.nextInt(1, this.size / 4);
            else
                variable = rnd.nextInt((this.size / 4) * 3, this.size - 2);
            return variable;
        }
        ...
        public void print() {
            for (int i = 0; i < this.size; i++) {
                for (int j = 0; j < this.size; j++) {
                    if (this.start[0] == j && this.start[1] == i)
                        System.out.print(" S ");
                    else if (this.finish[0] == j && this.finish[1] == i)
                        System.out.print(" F ");
                    else if (this.cell[j][i].homeRun)
                        System.out.printf("%3d", this.cell[j][i].step);
                    else {
